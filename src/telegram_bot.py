...
# Импортируем logger из core, чтобы использовать единый логгер
...
class TelegramHRBot:
    ...
    def __init__(self, token: str, flask_app: Flask, db):
        ...
        # УДАЛЕНО: Инициализация БД (create_engine, sessionmaker) - теперь db передается извне
        ...
        self.logger.info("TelegramHRBot инициализирован")
    ...
    def get_or_create_user(self, telegram_user):
        ...
        with self.app.app_context():
            user = self.db.session.query(User).filter_by(telegram_id=telegram_user.id).first()
            if not user:
                user = User(
                ...
                self.logger.info(f"Создан новый пользователь: {user.telegram_id}")
            else:
    ...
    def get_user(self, telegram_id):
        ...
        with self.app.app_context():
            return self.db.session.query(User).filter_by(telegram_id=telegram_id).first()
    ...
    def show_job_details(self, call, job_id):
        ...
        try:
            ...
            with self.app.app_context():
                job = self.db.session.query(Job).get(job_id)
                if not job:
    ...
    def view_application_details(self, call, application_id):
        ...
        try:
            with self.app.app_context():
                application = self.db.session.query(Application).get(application_id)
                if not application:
                    self.bot.answer_callback_query(call.id, "Отклик не найден")
                    return
                
                job = self.db.session.query(Job).get(application.job_id)
                applicant = self.db.session.query(User).get(application.applicant_id)
...
📋 <b>Команды для соискателей:</b>\n\n<b>Поиск работы:</b>\n/jobs - Все вакансии\n/search - Поиск вакансий\n/myapps - Мои отклики\n\n<b>Уведомления:</b>\n/subscribe - Подписаться на уведомления\n/subscriptions - Мои подписки\n\n<b>Быстрые действия:</b>\n/quick - Быстрые действия\n/stats - Моя статистика\n\n<b>Общие команды:</b>\n/profile - Мой профиль\n/settings - Настройки\n/menu - Главное меню\n/help - Эта справка\n                """
    ...
    def show_jobs_list(self, message, page=1):
        ...
        with self.app.app_context():
            ...
            jobs = self.db.session.query(Job).filter_by(is_active=True).offset((page-1)*self.JOBS_PER_PAGE).limit(self.JOBS_PER_PAGE).all()
            total_jobs = self.db.session.query(Job).filter_by(is_active=True).count()
    ...
    def show_employer_jobs(self, message):
        ...
        with self.app.app_context():
            ...
            jobs = self.db.session.query(Job).filter_by(employer_id=user.id, is_active=True).all()
            ...
            for job in jobs:
                # Считаем отклики
                applications_count = self.db.session.query(Application).filter_by(job_id=job.id).count()
    ...
    def show_job_applications(self, message):
        ...
        with self.app.app_context():
            ...
            all_applications = []
            for job in jobs:
                applications = self.db.session.query(Application).filter_by(job_id=job.id).all()
                for app_obj in applications:
                    app_obj.job = job  # Добавляем информацию о вакансии
                    all_applications.append(app_obj)
            ...
            for app_obj in all_applications:
                ...
                markup.add(
                    types.InlineKeyboardButton(
                        f"{status_emoji} {applicant.get_full_name()[:20]}... → {app_obj.job.title[:15]}...",
                        callback_data=f"view_application_{app_obj.id}"
                    )
                )
    ...
    def show_my_applications(self, message):
        ...
        with self.app.app_context():
            ...
            applications = self.db.session.query(Application).filter_by(applicant_id=user.id).all()
    ...
    def show_employer_stats(self, message):
        ...
        with self.app.app_context():
            ...
            # Получаем статистику
            total_jobs = self.db.session.query(Job).filter_by(employer_id=user.id, is_active=True).count()
    ...
    def handle_callback_query(self, call):
        ...
        try:
            ...
            self.logger.error(f"Ошибка в handle_callback_query: {e}")
...
ℹ️ <b>О HR Bot</b>\n\nЭтот бот поможет вам в поиске работы и подборе персонала.\n\n<b>Для соискателей:</b>\n• Поиск актуальных вакансий\n• Быстрые отклики на интересные предложения\n• Уведомления о новых вакансиях\n• Персональные рекомендации\n\n<b>Для работодателей:</b>\n• Размещение вакансий\n• Управление откликами\n• Статистика по вакансиям\n• Поиск кандидатов\n\nВыберите свою роль и начните использовать бота!\n        """
    ...
    def start_polling(self):
        ...
        self.logger.info("Запуск Telegram-бота в режиме polling...")
